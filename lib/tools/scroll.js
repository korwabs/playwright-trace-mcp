...
const scroll = (0, tool_1.defineTool)({
    capability: 'core',
    schema: {
        name: 'browser_scroll',
        description: '브라우저 페이지에서 스크롤을 수행합니다',
        inputSchema: zod_1.z.object({
            x: zod_1.z.number().optional().describe('가로 스크롤 양 (픽셀)'),
            y: zod_1.z.number().describe('세로 스크롤 양 (픽셀)'),
            behavior: zod_1.z.enum(['auto', 'smooth']).optional().describe('스크롤 동작 (기본값: auto)'),
        }),
    },
...
const scrollToElement = (0, tool_1.defineTool)({
    capability: 'core',
    schema: {
        name: 'browser_scroll_to_element',
        description: '지정된 요소로 스크롤하여 화면에 표시합니다',
        inputSchema: zod_1.z.object({
            element: zod_1.z.string().describe('스크롤할 요소에 대한 설명'),
            ref: zod_1.z.string().describe('페이지 스냅샷에서의 정확한 요소 참조'),
            behavior: zod_1.z.enum(['auto', 'smooth']).optional().describe('스크롤 동작 (기본값: auto)'),
            block: zod_1.z.enum(['start', 'center', 'end', 'nearest']).optional().describe('세로 정렬 (기본값: center)'),
            inline: zod_1.z.enum(['start', 'center', 'end', 'nearest']).optional().describe('가로 정렬 (기본값: nearest)'),
        }),
    },
    handle: async (context, params) => {
...
const scrollToPosition = (0, tool_1.defineTool)({
    capability: 'core',
    schema: {
        name: 'browser_scroll_to_position',
        description: '페이지의 특정 위치로 스크롤합니다',
        inputSchema: zod_1.z.object({
            x: zod_1.z.number().describe('가로 스크롤 위치 (픽셀)'),
            y: zod_1.z.number().describe('세로 스크롤 위치 (픽셀)'),
            behavior: zod_1.z.enum(['auto', 'smooth']).optional().describe('스크롤 동작 (기본값: auto)'),
        }),
    },
    handle: async (context, params) => {
...
const autoScroll = (0, tool_1.defineTool)({
    capability: 'core',
    schema: {
        name: 'browser_auto_scroll',
        description: '페이지 끝까지 자동으로 스크롤하여 전체 콘텐츠를 로드합니다',
        inputSchema: zod_1.z.object({
            distance: zod_1.z.number().optional().describe('각 스크롤 단계의 거리 (픽셀) (기본값: 100)'),
            delay: zod_1.z.number().optional().describe('스크롤 단계 사이의 지연 시간 (밀리초) (기본값: 100)'),
        }),
    },
    handle: async (context, params) => {
