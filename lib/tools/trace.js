...
const trace_viewer_1 = require("../utils/trace-viewer");
const traceStart = (0, tool_1.defineTool)({
    capability: 'core',
    schema: {
        name: 'browser_trace_start',
        description: '브라우저 세션의 트레이스 기록을 시작합니다',
        inputSchema: zod_1.z.object({
            directory: zod_1.z.string().optional().describe('트레이스를 저장할 디렉토리 경로(기본값: mcp_traces)'),
            name: zod_1.z.string().optional().describe('트레이스 파일 이름(기본값: trace)'),
            screenshots: zod_1.z.boolean().optional().describe('스크린샷 기록 여부(기본값: true)'),
            snapshots: zod_1.z.boolean().optional().describe('DOM 스냅샷 기록 여부(기본값: true)'),
        }),
    },
    handle: async (context, params) => {
        // 트레이스 디렉토리 설정
        const traceDir = params.directory || path_1.default.join(process.cwd(), 'mcp_traces');
...
const traceStop = (0, tool_1.defineTool)({
    capability: 'core',
    schema: {
        name: 'browser_trace_stop',
        description: '브라우저 세션의 트레이스 기록을 중지하고 파일로 저장합니다',
        inputSchema: zod_1.z.object({
            path: zod_1.z.string().optional().describe('저장할 트레이스 파일 경로(기본값: 시작 시 지정한 디렉토리와 이름 사용)'),
        }),
    },
    handle: async (context, params) => {
        const traceDir = context.options.traceDir || path_1.default.join(process.cwd(), 'mcp_traces');
        const traceName = context.options.traceName || 'trace';
        const tracePath = params.path || path_1.default.join(traceDir, `${traceName}.zip`);
        const code = [
...
const traceView = (0, tool_1.defineTool)({
    capability: 'core',
    schema: {
        name: 'browser_trace_view',
        description: '저장된 트레이스 파일을 Playwright Trace Viewer에서 엽니다',
        inputSchema: zod_1.z.object({
            path: zod_1.z.string().describe('열 트레이스 파일 경로'),
            port: zod_1.z.number().optional().describe('Trace Viewer를 실행할 포트(기본값: 9322)'),
        }),
    },
    handle: async (context, params) => {
        ...
        return {
            ...
            action: async () => {
                try {
                    const result = await (0, trace_viewer_1.showTraceViewer)(tracePath, port);
                    if (result.success) {
                        return {
                            content: [{
                                    type: "text",
